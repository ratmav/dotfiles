...
@test "ish_sqlite_dump outputs INSERT statements" {
  ...
  run ish_sqlite_dump --db="$db"
  assert_success
  assert_output --partial "INSERT INTO"
  assert_output --partial "'foo'"
}

@test "ish_sqlite_dump excludes schema" {
  local db="${ISH_TEST_FIXTURES}/test.db"
  mkdir -p "$ISH_TEST_FIXTURES"
...
@test "ish_sqlite_dump filters by table" {
  local db="${ISH_TEST_FIXTURES}/test.db"
  mkdir -p "$ISH_TEST_FIXTURES"
  ...
  run ish_sqlite_dump --db="$db" --tables="items"
  assert_success
  assert_output --partial "'foo'"
  refute_output --partial "'bar'"
}

@test "ish_sqlite_dump fails on empty database" {
  local db="${ISH_TEST_FIXTURES}/test.db"
  mkdir -p "$ISH_TEST_FIXTURES"
...
@test "ish_sqlite_dump fails on missing database" {
  run ish_sqlite_dump --db="/tmp/ish_fixtures/nonexistent.db"
  assert_failure
  assert_output --partial "dump: database not found"
}

@test "ish_sqlite_dump requires --db=" {
  run ish_sqlite_dump
  assert_failure
  assert_output --partial "dump: --db= required"
}

@test "ish_sqlite_dump rejects unknown options" {
  run ish_sqlite_dump --foo=bar
  assert_failure
