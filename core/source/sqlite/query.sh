...
ish_sqlite_query() {
  ...
  while [[ $# -gt 0 ]]; do
    case $1 in
      --db=*) db="${1#*=}"; shift ;;
      --sql=*) sql="${1#*=}"; shift ;;
      *) ish_sqlite_error "query: unknown option: $1" ;;
    esac
  done

  [[ -z "$db" ]] && ish_sqlite_error "query: --db= required"
  [[ -z "$sql" ]] && ish_sqlite_error "query: --sql= required"

  ish_sqlite_require

  sqlite3 -separator '|' "$db" "PRAGMA foreign_keys = ON; ${sql}" \
    || ish_sqlite_error "query failed: ${sql}"
}
