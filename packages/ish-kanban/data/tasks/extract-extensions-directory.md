...
- [ ] move git.sh + git/ and sqlite3.sh + sqlite3/ from core/source/ to extensions/source/
- [ ] update routers to source foundations via `${ISH_CORE}/source/` instead of `${module_dir}/`
- [ ] add `ISH_INTEGRATIONS` to core/bin/ish
- [ ] move git and sqlite3 test files from core/test/unit/ to extensions/test/unit/
...
- [ ] update kanban tasks (build-fp-core-git.md, split-fp-core-sqlite3.md)
- [ ] run all tests, verify passing

## deliverable

`extensions/` as a top-level directory with git and sqlite3 source + tests. core contains only foundation and primitives. dependency direction is explicit.
