...
ish_ratfiles_nix_semantic() {
  ...
  if [[ -z "$package" ]] || [[ -z "$version" ]]; then
    ish_tui_error --message="package name and version are required."
  fi

  ish_tui_info --message="searching for $package version $version in nixpkgs..."

  if ! found_path=$(_nix_find_package_path "$package"); then
    ish_tui_error --message="...could not find package '$package'."
  fi

  ish_tui_info --message="...found $package $version at: $found_path"

  if ! found_commit=$(_nix_find_version_commit "$package" "$version" "$found_path"); then
    ish_tui_error --message="...did not find $package version $version in recent commits."
  fi

  ish_tui_info --message="...found $package $version at commit $found_commit"
...
ish_ratfiles_nix_route() {
  ...
  help|"")
    ...
    ;;
  *)
    ish_tui_error --message="unknown nix command: ${1-}"
    ish_ratfiles_nix_help
    return 1
    ;;
  esac
...
_nix_find_package_path() {
  ...
  for path in "${possible_paths[@]}"; do
    if ish_tui_quiet "curl -s -f https://api.github.com/repos/NixOS/nixpkgs/contents/$path"; then
      ish_tui_info --message="$path"
      return 0
    fi
  done
  return 1
...
with pkgs; [
  ...
  ish_tui_template "${ISH_TUI_INFO}" "$template" package "$package" version "$version" commit "$commit"
}
