...
ish_ratfiles_bootstrap_posix_git_configure() {
  ...
  if ! ish_utils_exists_executable --executable=nvim; then
    ...
    ish_tui_info --message="${FUNCNAME[0]}: configured git editor."
  fi

  git config --global core.excludesfile "$HOME/.gitignore_global"
  ish_tui_info --message="${FUNCNAME[0]}: configured global gitignore."

  git config --global push.autoSetupRemote true
  ish_tui_info --message="${FUNCNAME[0]}: configured git to automatically setup remote branches on push."
}
