...
@test "ish_sqlite3_query_one returns single row" {
  ...
  run ish_sqlite3_query_one --db="$db" --sql="SELECT name FROM items;"
  assert_success
  assert_output "foo"
}

@test "ish_sqlite3_query_one returns pipe-separated columns" {
  local db="${ISH_TEST_FIXTURES}/test.db"
  mkdir -p "$ISH_TEST_FIXTURES"
  ...
  run ish_sqlite3_query_one --db="$db" --sql="SELECT name, status FROM items;"
  assert_success
  assert_output "foo|open"
}

@test "ish_sqlite3_query_one fails on zero rows" {
  local db="${ISH_TEST_FIXTURES}/test.db"
  mkdir -p "$ISH_TEST_FIXTURES"
...
@test "ish_sqlite3_query_one fails on multiple rows" {
  local db="${ISH_TEST_FIXTURES}/test.db"
  mkdir -p "$ISH_TEST_FIXTURES"
...
@test "ish_sqlite3_query_one requires --db=" {
  run ish_sqlite3_query_one --sql="SELECT 1;"
  assert_failure
  assert_output --partial "query_one: --db= required"
}

@test "ish_sqlite3_query_one requires --sql=" {
  run ish_sqlite3_query_one --db="/tmp/test.db"
  assert_failure
