...
@test "ish_sqlite3_transaction commits on success" {
  ...
  run ish_sqlite3_transaction --db="$db" <<< "INSERT INTO items (name) VALUES ('foo'); INSERT INTO items (name) VALUES ('bar');"
  assert_success
...
@test "ish_sqlite3_transaction rolls back on failure" {
  local db="${ISH_TEST_FIXTURES}/test.db"
  mkdir -p "$ISH_TEST_FIXTURES"

  sqlite3 "$db" "CREATE TABLE items (id INTEGER PRIMARY KEY, name TEXT NOT NULL);"

  run ish_sqlite3_transaction --db="$db" <<< "INSERT INTO items (name) VALUES ('foo'); INSERT INTO items (name) VALUES (NULL);"
  assert_failure
...
@test "ish_sqlite3_transaction enforces foreign keys" {
  local db="${ISH_TEST_FIXTURES}/test.db"
  mkdir -p "$ISH_TEST_FIXTURES"
  ...
  run ish_sqlite3_transaction --db="$db" <<< "INSERT INTO children (parent_id) VALUES (999);"
  assert_failure
...
@test "ish_sqlite3_transaction requires --db=" {
  run ish_sqlite3_transaction <<< "SELECT 1;"
  assert_failure
