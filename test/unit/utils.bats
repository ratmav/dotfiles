...
@test "utils_stream_stdout writes to stdout" {
  run bash -c 'source bash/utils.sh; utils_stream_stdout "test" >/tmp/stdout.txt 2>/tmp/stderr.txt; cat /tmp/stdout.txt'
  assert_output "test"
...
@test "utils_stream_stderr writes to stderr" {
  run bash -c 'source bash/utils.sh; utils_stream_stderr "test" >/tmp/stdout.txt 2>/tmp/stderr.txt; cat /tmp/stderr.txt'
  assert_output "test"
...
@test "utils_stream_stdout handles dash prefixed arguments" {
  run bash -c 'source bash/utils.sh; utils_stream_stdout "-n"'
  assert_output "-n"
}

@test "utils_stream_stderr handles dash prefixed arguments" {
  run bash -c 'source bash/utils.sh; utils_stream_stderr "-n" 2>&1'
  assert_output "-n"
}
...
@test "utils_stream_multiline_stdout outputs multiple lines to stdout" {
  run bash -c 'source bash/utils.sh; utils_stream_multiline_stdout <<EOF
line one
line two
line three
EOF'
...
@test "utils_stream_multiline_stdout preserves blank lines" {
  run --keep-empty-lines bash -c 'source bash/utils.sh; printf "%s\n%s\n%s\n" "first" "" "second" | utils_stream_multiline_stdout'
  assert_success
...
@test "utils_stream_multiline_stderr outputs multiple lines to stderr" {
  run bash -c 'source bash/utils.sh; utils_stream_multiline_stderr <<EOF
line one
line two
line three
EOF' 2>&1
